...
export default function NeetworkBlock() {
            ...
            if (prevSelectedFeatureIds[activeTab]) {
                ...
                if (prevSelectedFeatureIds[activeTab].includes(featureId)) {
                    return {
                        ...
                        [activeTab]: prevSelectedFeatureIds[activeTab].filter((id) => id !== featureId),
                    };
                } else {
                    return {
                        ...prevSelectedFeatureIds,
    ...
    const handleOptionChange = (featureId, selectedOption) => {
        const cloneSelectedRadioOption = selectedRadioOption && selectedRadioOption.length > 0 ? [...selectedRadioOption] : [];

        if (CommonUtility.isValidArray(cloneSelectedRadioOption)) {
            const availableDataIndex = cloneSelectedRadioOption.findIndex((item) => item.featureId === featureId && item.activeTab === activeTab);
                                        ...
                                        if (headerOption.active) {
                                            return (
                                                <Nav.Item key={headerOption.id} className="me-3 me-lg-0 mt-3 mt-lg-0">
                                                    <Nav.Link className="mdf__btn_large" eventKey={headerOption.id}>
                                                        {headerOption.name}
                                                    </Nav.Link>
                                                </Nav.Item>
                                            );
                                                    ...
                                                    {headerOption.features?.map((item) => (
                                                        <Col md={6} xl={4} className="mt-4" key={item.name}>
                                                            <Form.Control
                                                                ...
                                                                hidden
                                                                checked={selectedFeatureIds[activeTab]?.includes(item.webFeatureKey)}
                                                                onChange={() => {
                                                                    if (!item.active) {
                                                                        newInfoAlert(
                                                            ...
                                                            >
                                                                <div>
                                                                    <img className={`lazyload ${style.mdf__card_img}`} src={item.imgUrl} alt="A globe icon with filter and text." />
                                                                </div>
                                                                                ...
                                                                                <input
                                                                                    ...
                                                                                    checked={
                                                                                        ...
                                                                                        selectedRadioOption.findIndex(
                                                                                            (a) => a.selectedOption === opt.name && a.activeTab === activeTab && a.featureId === item.webFeatureKey
                                                                                        ) !== -1
                                                                                    }
                                                                                    name={`radio-${item.featureId}-${activeTab}`}
                                                                                    onChange={() => handleOptionChange(item.webFeatureKey, opt.name)}
                                                                                    id={activeTab + item.featureId + opt.name.replace(/\s/g, "")}
                                                                                    autoComplete="off"
                                                                                />
                                                                                <label className="btn btn-outline-dark px-2 text-xs" htmlFor={activeTab + item.featureId + opt.name.replace(/\s/g, "")}>
                                                                                    {opt.name}
                                                                                </label>
                                                                            </div>
                                                                        );
