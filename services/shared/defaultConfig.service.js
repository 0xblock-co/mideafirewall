...
export const asyncGetAllHeaderData = createAsyncThunk("GET_ALL_HEADER_DATA_OPTIONS", async (payload, thunkAPI) => {
    try {
        ...
        const response = api.get(`https://mediafirewall.themillionvisions.com/mfw/web/customerCategories?${queryStringData}`, {}, false, false).then(async (res) => {
            if (res && res?.isSuccess) {
                ...
                if (res.data && CommonUtility.isValidArray(res.data.items)) {
                    ...
                    const isDuplicateFeature = (features, feature) => features.some((existingFeature) => existingFeature.featureId === feature.featureId && existingFeature.name === feature.name);
