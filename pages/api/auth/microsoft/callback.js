...
export default async (req, res) => {
    ...
    try {
        ...
        if (tokenData && "id_token" in tokenData) {
            res.redirect(`/account-security/login?authType=microsoft&success=true&value=${tokenData.id_token}`);
        } else {
            res.redirect(`/account-security/login?authType=microsoft&success=false&value=`);
        }
    } catch (error) {
