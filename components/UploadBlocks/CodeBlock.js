                ...
                {codeData.map((tab, index) => {
                    return (
                        <Tab key={index} eventKey={index} title={tab.title}>
                            ...
                            <p>{highlightCode(CommonUtility.replaceApiSecretAndFilters(tab.urlSnippets, user?.userDetails?.email, user?.api_secret, router.query.filters), tab.title)}</p>
                            <h6 className="mt-3 mb-3">Moderate by submitting file</h6>
                            <p>{highlightCode(CommonUtility.replaceApiSecretAndFilters(tab.fileSnippets, user?.userDetails?.email, user?.api_secret, router.query.filters), tab.title)}</p>
                        </Tab>
                    );
                })}
            </Tabs>
